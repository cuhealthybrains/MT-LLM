...
class SpeechLLMDataset(FairseqDataset):
    ...
    def __getitem__(self, index: int) -> SpeechLLMDatasetItem:
        ...
        # # targetASR for training
        # if "audio file starts with a 3-second reference speech by the target speaker" in self.prompt_texts[index]:
        #     spks = self.spks[index].split("|")
        #     select_spk_index = random.choice(range(len(spks)))
        #     select_spk = spks[select_spk_index]
        #     select_num = random.choice(range(5))
        #     ref_audio_path = f"examples/wavllm/test_data/reference_enroll_audio/{select_spk}/{select_spk}_{select_num}.wav"
        #     ref_audio = self._get_few_shot_audio(ref_audio_path)  
        #     num_samples = self.cfg.use_sample_rate * 3.0  # 3 seconds
        #     silent_padding = torch.zeros(num_samples, dtype=torch.float32) 
        #     source = torch.cat((ref_audio, silent_padding, source))
        ...
        if self.cfg.alpaca_text:
            ...
            return SpeechLLMDatasetItem(
                ...
                orig_prompt=orig_prompt, wavlm_sources=wavlm_sources,
